...
const fetchData = () => {
    axios.get("https://63c61ff54ebaa802853ff786.mockapi.io/api/todoItems")
    .then((response) => {
        main(response.data)
    })
    .catch((err) => {
...
function li_activeItem(item) {
    ...
    btn.onclick = function(e) {
        ...
        const res = axios.delete(`https://63c61ff54ebaa802853ff786.mockapi.io/api/todoItems/${id}`)
        .then((response) => {
...
const addItem = (item) => {
    axios.post("https://63c61ff54ebaa802853ff786.mockapi.io/api/todoItems", item)
    .then((response) => {
...
$('.todo-list').addEventListener('click', function(e) {
    if (e.target.className === 'toggle') {
        ...
        if(e.target.checked === false){
            ...
            const res = axios.put(`https://63c61ff54ebaa802853ff786.mockapi.io/api/todoItems/${id}`, {
                status: 'active'
            })
            .then((response) => {
                if(response.status === 200){
